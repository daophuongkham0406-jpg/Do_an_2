...
async function handleRegister(e) {
    ...
    if (!passRegex.test(pass)) {
        return showToast("⚠️ Mật khẩu phải từ 8 ký tự, gồm ít nhất 1 chữ hoa, 1 chữ thường và 1 số!", "error");
    }
    ...
    try {
        ...
        showToast("🔌 Không thể kết nối tới server. Vui lòng bật Backend!", "error");
    }
}
...
document.getElementById('close-otp').addEventListener('click', () => otpModal.classList.remove('show'));

document.getElementById('btn-verify-reg').addEventListener('click', async () => {
    const otpCode = document.getElementById('reg-otp-input').value.trim();
    if (otpCode.length !== 6) return showToast("⚠️ Vui lòng nhập đủ 6 số OTP", "error");
...
document.getElementById('close-forgot').addEventListener('click', () => forgotModal.classList.remove('show'));
...
document.getElementById('btn-send-reset').addEventListener('click', async () => {
    ...
    finally { btn.innerText = "Gửi mã OTP"; btn.disabled = false; }
});
...
document.getElementById('btn-confirm-reset').addEventListener('click', async () => {
    ...
    if (otpCode.length !== 6) return showToast("⚠️ Mã OTP phải gồm 6 số!", "error");
    ...
    if (newPass !== confirmPass) return showToast("⚠️ Mật khẩu xác nhận không khớp!", "error");

    try {
        const res = await fetch('http://127.0.0.1:5000/api/auth/reset-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: window.tempResetEmail, otp: otpCode, newPassword: newPass })
        });
